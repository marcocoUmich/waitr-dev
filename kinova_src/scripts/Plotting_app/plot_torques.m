%%
function plot_torques(plt, input, input_centers, input_radii, time, trajectory, bounds)
    ...
    for i = 1:iterations
        ...
        for j = 1:JOINTS
            ...
            subplot(ceil(JOINTS/2), 2, j, 'Parent', plt);
            ...
            if bounds ~= -1
                ...
                plot(plt, x, repmat(upper_limit, 1, length(x)), 'k');
                plot(plt, x, repmat(lower_limit, 1, length(x)), 'g');
